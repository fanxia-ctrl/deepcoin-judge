...
THEMES: tuple[Theme, ...] = (
    ...
    Theme("ontopic", "T2", "切题与完整性",
          ...
          hint="把用户这句话真正想知道的事说清楚了没有；对不对不在本主题。"
               ...
               "本主题与 T5 独立：一条路径既可以无据（T5）也同时笼统（neg_2_4），两边都要判。",
          tips=(
              ...
              Tip("neg_2_3", "只给结论不带规则",
                  "给了结论却没带背后的规则或依据，不限问法：问「为什么」只说「就是这样」；"
                  "问「能不能关」只答「不能」，不说是默认设置、何时解冻、在哪查。"
                  "结论对不对不在这里判", NORMAL, 6, 0.17,
                  note="第一批复核放宽：原只限「问为什么」，产品挑出的 8 条完整性漏判都是"
                       "非为什么问法的结论没带规则"),
              Tip("neg_2_4", "笼统到不可执行",
    ...
    Theme("evidence_ask", "T3", "个人账户类问题缺取证",
          ...
          hint="这一问是否指向用户自己的某一单、某个仓位、某笔资金。如果是，回答该先取证再判断。"
               "语音通道不支持图片，索要截图不算取证。"
               "用户自己陈述的账户情况（「我返点有 4U」）可以作为该账户事实的依据，"
               "回答顺着它说不算未取证；但它不能作为平台规则的依据。",
          tips=(
