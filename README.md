...
**输入**，每条六样材料：原始问题 · 回答 · 本轮证据 · 路由业务规则 · 权威口径 · 机器信号。
不是每个主题都吃全部，缺了就不判。

其中「本轮证据」和「路由业务规则」是**被测模型这一轮实际看到的原文**，从 Dify 的
`final_context` 节点取，不是另拼的：
- 本轮证据 = `Knowledge evidence`（KB 切片）+ `Live account evidence`（查账户/行情接口的返回）
- 路由业务规则 = system prompt 里按路由固定的那 8K 业务口径

这样才分得开三种错：KB 没召回 → 检索问题；KB/工具给对了模型说错 → 模型问题（T1）；
业务规则与 KB 打架 → KB 问题（不罚模型）。
...
**默认 `one`：一次把全部材料和 14 条判据发出去，一次拿回 14 条 verdict。**
...
judge/
  ...
  context.py           组装材料（证据以 evidence_text 原文为准，老数据退回 kb_hits）
...
**工具记录只有 26/94 轮。** 已接上（`tool_calls`，含返回是否为空），R9/R10 生效。
但另外 68 轮模型根本没调接口，其中不少是个人账户类问题 —— 这本身就是 `neg_3_1` 要抓的。
